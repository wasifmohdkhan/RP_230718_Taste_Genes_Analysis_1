...
Sample538_16S_ITS_lasso_ECC_assoc_to_gene_variant_3 = Sample538_16S_ITS_lasso_ECC_assoc_to_gene_variant %>% 
  ...
  dplyr::select( Gene, feature, SNP, Location_GRCh38, Effect_allele, Variant_Type,Variant_Effect, EFFECT,
                 # P, OR, p_BH
...
# make a complexheatmap for snps and microbiome
Prepapre the data

```{r} 
Sample538_16S_ITS_lasso_ECC_assoc_to_gene_variant_5 = Sample538_16S_ITS_lasso_ECC_assoc_to_gene_variant_3 %>% 
  dplyr::select( Gene, feature, Location_GRCh38, Variant_Type,EFFECT) %>%
  mutate(Gene_Location = paste0(Gene, "_", Location_GRCh38)) %>% 
  dplyr::select(-Gene, -Location_GRCh38) %>% 
  relocate(Gene_Location, feature, Variant_Type, EFFECT) %>% 
  arrange(Gene_Location)
Sample538_16S_ITS_lasso_ECC_assoc_to_gene_variant_5
```
```{r}
# Convert to wide format for heatmap
# Pivot data to wide format for heatmap
heatmap_data <- Sample538_16S_ITS_lasso_ECC_assoc_to_gene_variant_5 %>%
  pivot_wider(names_from = feature, values_from = EFFECT, values_fill = list(EFFECT = 0))
heatmap_data

# Convert to matrix
heatmap_matrix <- heatmap_data  %>% 
  column_to_rownames("Gene_Location") %>%
  select(-Variant_Type) %>% 
  as.matrix()
heatmap_matrix
...
t_sig_matrix <- t(sig_matrix)
```
```{r}
# Create a unique mapping of Gene_Location to Variant_Type
variant_type_mapping <- Sample538_16S_ITS_lasso_ECC_assoc_to_gene_variant_5 %>% 
  select(Gene_Location, Variant_Type) %>% 
  distinct()
variant_type_mapping

# Ensure the order matches the columns in the heatmap matrix
variant_types_ordered <- variant_type_mapping$Variant_Type[match(rownames(heatmap_matrix), variant_type_mapping$Gene_Location)]
variant_types_ordered
```


```{r}
# Create the annotation
variant_type_annotation <- HeatmapAnnotation(Variant_Type = variant_types_ordered,
                                             col = list(Variant_Type = c("synonymous_variant" = "blue",
                                                                         "frameshift_variant" = "red",
                                                                         "intron_variant" = "green",
                                                                         "missense_variant" = "purple",
                                                                         "3_prime_UTR_variant" = "orange",
                                                                         "splice_region_variant&intron_variant" = "yellow",
                                                                         "downstream_gene_variant" = "grey")))
...
color_mapping = colorRamp2(c(-max(abs(heatmap_matrix)), 0, max(abs(heatmap_matrix))), c("blue", "white", "red"))

# Prepare the heatmap object with annotations and border settings
ht <- Heatmap(t_heatmap_matrix,
              name = "EFFECT",
              bottom_annotation = variant_type_annotation,
              col = color_mapping,
              cluster_rows = TRUE,
              cluster_columns = FALSE,
              show_row_names = TRUE,
              show_column_names = TRUE,
              show_row_dend = FALSE,  # Disable the row dendrogram
              row_names_side = "left",  # Position row labels on the left
              column_title = "Gene_Location",
              column_title_side = "bottom",
              row_title = "Feature",
              rect_gp = gpar(col = "grey", lwd = 1),
              row_names_max_width = max_text_width(
                rownames(t_heatmap_matrix),
                gp = gpar(fontsize = 12)
              ),
              ...
              cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
                grid.text(t_sig_matrix[i, j], x, y)
              }
...
# Saving as PNG
png(here("Samples538_Analysis/Samples538_A05_Plink_Microbiome/Sample538_55genes_microbiome_heatmap.png"),
    width = 1400, height = 1000)
draw(ht,
     heatmap_legend_side = "top", 
     annotation_legend_side = "right",
     # heatmap_legend_height = unit(10, "cm"),
     # annotation_legend_height = unit(5, "cm")
     )
dev.off()

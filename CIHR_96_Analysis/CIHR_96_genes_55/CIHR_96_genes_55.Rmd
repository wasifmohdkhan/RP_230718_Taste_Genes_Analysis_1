...
process_MarkDuplicates() {
  ...
  # rmdup did not remove any reads so trying markdup
    ...
    samtools fixmate -m - - | \
    samtools sort - | \
    samtools markdup -r - ${SAMPLE_NAME}_sorted_RG_markdup_55genes.bam \
    --verbosity 2
...
rm -r    ./sorted_RG_markdup_55genes_bamqc/Qualimap
mkdir -p ./sorted_RG_markdup_55genes_bamqc/Qualimap

process_Qualimap() {
  SAMPLE_NAME=$(echo $1 | sed -e 's/_sorted_RG_markdup_55genes.bam//')
  
    _JAVA_OPTIONS="-Xms10g -Xmx30g" qualimap bamqc \
      -nt 1 \
      -bam ${SAMPLE_NAME}_sorted_RG_markdup_55genes.bam \
      -outdir ./sorted_RG_markdup_55genes_bamqc/Qualimap/${SAMPLE_NAME}_sorted_RG_markdup_55genes_qualimap \
      --genome-gc-distr hg19 \
...
export -f process_Qualimap
# Run the function in parallel for all *.bam files
ls *_sorted_RG_markdup_55genes.bam     | parallel -j 8 process_Qualimap

## MultiQC
/home/mwkhan/anaconda3/envs/Multiqc_env/bin/multiqc \
--interactive --force \
./sorted_RG_markdup_55genes_bamqc/Qualimap \
-o ./sorted_RG_markdup_55genes_bamqc/Qualimap
```
...
process_gatk_BaseRecalibrator() {
  ...
  _JAVA_OPTIONS="-Xmx90g" gatk ApplyBQSR \
      ...
      -O ${SAMPLE_NAME}_sorted_RG_markdup_BQSR_55genes.bam
}

export -f process_gatk_BaseRecalibrator
ls   *_sorted_RG_markdup_55genes.bam   | parallel -j 20 process_gatk_BaseRecalibrator
```
...
cd        /home/mwkhan/Taste_Genes_Analysis/CIHR_96_Analysis/CIHR_96_genes_55/CIHR_96_55_GATK/
mkdir -p  /home/mwkhan/Taste_Genes_Analysis/CIHR_96_Analysis/CIHR_96_genes_55/CIHR_96_55_GATK/sorted_RG_markdup_BQSR_HaplotypeCaller_55genes

# Function for processing each file
process_gatk_HaplotypeCaller() {

  SAMPLE_NAME=$(echo $1 | sed -e 's/_sorted_RG_markdup_BQSR_55genes.bam//')
  ...
  _JAVA_OPTIONS="-Xmx90g" gatk HaplotypeCaller \
      ...
      -I $1 \
      -O sorted_RG_markdup_BQSR_HaplotypeCaller_55genes/${SAMPLE_NAME}_raw_variants.g.vcf.gz \
      --assembly-region-padding 100 \
...
export -f process_gatk_HaplotypeCaller
ls    *_sorted_RG_markdup_BQSR_55genes.bam  | parallel -j 20 process_gatk_HaplotypeCaller
